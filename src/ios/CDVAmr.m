...
- (void)loadInterstitial:(CDVInvokedUrlCommand *)command {
    ...
    // DEBUG: Gelen argümanları logla
    NSLog(@"<AMRSDK>[DEBUG] loadInterstitial arguments: %@", command.arguments);

    if (command.arguments.count > 0) {
        NSDictionary* params = [command argumentAtIndex:0 withDefault:[NSNull null]];
        // DEBUG: Çözümlenen parametreleri logla
        NSLog(@"<AMRSDK>[DEBUG] loadInterstitial parsed params: %@", params);
        // DEBUG: Beklenen bazı anahtarlar özelinde log
        if ([params isKindOfClass:[NSDictionary class]]) {
            NSLog(@"<AMRSDK>[DEBUG] interstitialIdIOS=%@ autoShowInterstitial=%@ canRequestAds=%@ userConsent=%@ subjectToGdpr=%@ subjectToCCPA=%@",
                  [params objectForKey:@"interstitialIdIOS"],
                  [params objectForKey:@"autoShowInterstitial"],
                  [params objectForKey:@"canRequestAds"],
                  [params objectForKey:@"userConsent"],
                  [params objectForKey:@"subjectToGdpr"],
                  [params objectForKey:@"subjectToCCPA"]);
        }
        [self __setOptions:params];
    }
...
-(void)didDismissInterstitial:(AMRInterstitial *)interstitial {
    ...
    // WebView layout'unu düzelt - view hierarchy düzelene kadar bekle
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self resizeContent];
        
        // Eğer banner varsa, tekrar ekle (z-index sorununu çöz)
        if (_banner && _bannerIsVisible) {
            UIView* parentView = _overlap ? self.webView : [self.webView superview];
            [parentView bringSubviewToFront:_banner.bannerView];
        }
